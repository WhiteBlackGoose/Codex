...
namespace Codex.Storage
{
    public class ElasticsearchStorage : IStorage, IAnalysisTarget
    {
        ...
        {
            return UploadAsync(repoFile.Repo.TargetIndex, boundSourceFile);
        }

        public async Task UploadAsync(string targetIndex, BoundSourceFile boundSourceFile)
        {
            await Provider.AddSourcesToIndexAsync(targetIndex, new[] { ModelConverter.FromObjectModel(boundSourceFile) });
        }
        ...
        public Task AddProjectAsync(IRepoProject repoProject, AnalyzedProject analyzedProject)
        {
            ...
            return AddProjectAsync(analyzedProject, targetIndex);
        }

        public async Task AddProjectAsync(AnalyzedProject analyzedProject, string targetIndex)
        {
        ...
        {
            string targetIndex = repo.TargetIndex;
            string repositoryName = repo.RepositoryName;
            await AddRepository(targetIndex, repositoryName);
        }

        public async Task AddRepository(string targetIndex, string repositoryName)
        {
            Console.WriteLine($"Adding repository {repositoryName} to index {targetIndex}");
            await Provider.AddRepositoryAsync(new RepositoryModel(repositoryName, targetIndex)
            {
        ...
        public Task FinalizeRepository(IRepo repo)
        {
            string targetIndex = repo.TargetIndex;
            string repositoryName = repo.RepositoryName;
            return FinalizeRepository(targetIndex, repositoryName);
        }

        public Task FinalizeRepository(string targetIndex, string repositoryName)
        {
            Console.WriteLine($"Finalizing repository {repositoryName} to index {targetIndex}");
            return Provider.FinalizeRepositoryAsync(repositoryName, targetIndex);
        }
    }
}